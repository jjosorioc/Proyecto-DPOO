...
public class PanelEstadisticas extends JPanel
{
	...
	public PanelEstadisticas(JFrame padreFrame, ArrayList<Integer> unidades, String producto,String fechas)
	{
		this.padre = padreFrame;
		this.title = this.title + producto;
		this.fechas = fechas.split(",");
		this.valoresEnOrden = unidades.toArray(new Integer[0]);
	...
	public void paintComponent(Graphics g)
	{
		...
		for (int i = 0; i < this.valoresEnOrden.length; i++)
		{
			...
			g.setColor(new java.awt.Color((int) (Math.random()*255), (int) (Math.random()*255), (int) (Math.random()*255)));
			g.fillRect(valueX, valueY, barWidth - 2, height);
			g.setColor(Color.black);
			g.drawRect(valueX, valueY, barWidth - 2, height);
			int labelWidth = labelFontMetrics.stringWidth(this.fechas[i]);
			x = i * barWidth + (barWidth - labelWidth) / 2;
			g.drawString(this.fechas[i], x, y);
		}
